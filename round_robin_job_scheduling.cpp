...
{
    public:
        int job_id, arrivalTime, burstTime, tempBurstTime, startTime, completionTime, waitingTime, turnAroundTime;
};
...
int main()
{
    ...
    cout << "\nJobs   Arrival Time   Burst Time   Start Time   Completion Time   Waiting Time   Turnaround Time" << endl;
    ...
    for(int i=0; i<totalJobs; i++)
        cout << job[i].job_id         << "\t\t" 
             ...
             << job[i].burstTime      << "\t\t" 
             << job[i].startTime      << "\t\t"
             << job[i].completionTime << "\t\t"
             << job[i].waitingTime    << "\t\t"
             << job[i].turnAroundTime << endl;
